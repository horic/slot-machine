...
public class Master extends UntypedActor {
	...
	public void preStart() throws Exception {
		...
		ActorRef agentRouter = getContext().actorOf(
				Props.create(SlotSearchAgent.class).withRouter(
						new RoundRobinRouter(7)), "agentRouter");
		...
		for (File file : dir.listFiles()) {
			agentRouter.tell(new SlotSearchAgentPayload(file, randomWord),
					getSelf());
			numOfAgents++;
		}
	}
